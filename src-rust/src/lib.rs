...
use model::{
    ChatMessage, CompletionParams, ContextParams, EmbedInput, EmbedRequest, EmbedResponse,
    GenerateRequest, GenerateResponse, ModelInitOptions,
};
...
pub fn load_model(model_id: String, bytes: &[u8], opts_json: String) -> Result<(), JsValue> {
    #[cfg(llama_embed_cpp)]
    {
        ...
        // Parse options to extract model path and parameters
...
pub fn generate(model_id: String, req_json: String) -> Result<String, JsValue> {
    #[cfg(llama_embed_cpp)]
    {
        ...
        // Build prompt from either direct prompt or messages
        ...
        if let Some(error) = completion_json
            .get("error")
            .and_then(|v| v.as_str())
        {
